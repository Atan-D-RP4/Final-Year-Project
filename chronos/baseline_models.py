...
try:
    ...
    print("Warning: statsmodels not available. Some baseline models will use simplified implementations.")
...
class SeasonalNaiveForecaster(BaselineForecaster):
    ...
    def fit(self, data: List[float]) -> 'SeasonalNaiveForecaster':
        ...
        if len(data) < self.season_length:
            raise ValueError(f"Data must have at least {self.season_length} observations")
        self.data = data.copy()
        self.is_fitted = True
        return self
...
class ExponentialSmoothingForecaster(BaselineForecaster):
    ...
    def __init__(self, alpha: float = 0.3, trend: Optional[str] = None, seasonal: Optional[str] = None, seasonal_periods: int = 12):
        ...
        if STATSMODELS_AVAILABLE:
            try:
                ...
                if self.seasonal and len(data) < 2 * self.seasonal_periods:
                    print(f"Warning: Not enough data for seasonal model. Using simple exponential smoothing.")
                    self.seasonal = None
                ...
                print(f"Warning: statsmodels exponential smoothing failed: {e}. Using simple implementation.")
...
class BaselineComparison:
    ...
    def compare_models(self, train_data: List[float], test_data: List[float], 
