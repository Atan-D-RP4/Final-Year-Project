...
class IMFDataLoader:
    ...
    def _parse_weo_response(
        self, response_data: Dict, country: str, indicator: str
    ) -> Optional[pd.DataFrame]:
    ...
    def get_inflation_data(
        ...
        for country in countries:
            df = self.fetch_weo_data(
                country, self.INDICATORS["inflation"], start_year, end_year
            )
            if df is not None and not df.empty:
    ...
    def get_gdp_growth_data(
        ...
        for country in countries:
            df = self.fetch_weo_data(
                country, self.INDICATORS["gdp_growth"], start_year, end_year
            )
            if df is not None and not df.empty:
