...
def demo_imf_data_integration():
    ...
    return {"inflation": inflation_data, "gdp_growth": gdp_data, "usd_eur": fx_data}
...
def demo_data_validation():
    ...
    print(f"  Quality Score: {validation['quality_score']:.2f}")
    print(
        f"  Statistics: Mean={validation['statistics']['mean']:.2f}, Std={validation['statistics']['std']:.2f}"
    )
...
def demo_feature_engineering():
    ...
    for i, config in enumerate(configs):
        ...
        print(
            f"Config {i + 1}: window={config['window_size']}, levels={config['quantization_levels']}"
        )
