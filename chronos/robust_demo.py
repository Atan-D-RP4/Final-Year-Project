...
def main():
    ...
    print(
        f"✅ Data range: [{gdp_validation['statistics']['min']:.2f}%, {gdp_validation['statistics']['max']:.2f}%]"
    )
    ...
    tokenizer = BehavioralDataTokenizer(
        window_size=window_size, quantization_levels=1000
    )

    try:
        tokenized_inflation = tokenizer.fit_transform(inflation_data)
        print(
            f"✅ Tokenized inflation: {len(inflation_data)} → {len(tokenized_inflation)} tokens"
        )
        print(
            f"✅ Token range: [{tokenized_inflation.min():.1f}, {tokenized_inflation.max():.1f}]"
        )
        ...
        print("✅ Fallback tokenization successful")
    ...
    try:
        ...
        if len(train_data) >= forecast_window * 2:  # Ensure sufficient data
            tokenized_data, _ = forecaster.prepare_data(
                train_data, window_size=forecast_window
            )
            ...
            for i, (actual, pred) in enumerate(zip(test_data, predictions)):
                ...
                print(
                    f"   Year {i + 1}: Actual={actual:.2f}%, Predicted={pred:.2f}%, Error={error:.2f}%"
                )
            ...
            mape = (
                np.mean(
                    np.abs(
                        (np.array(test_data) - predictions)
    ...
    datasets = {
        ...
        "Global_Economic_Index": [
            50 + 10 * np.sin(i / 4) + np.random.normal(0, 2) for i in range(n_years)
        ],
    }
    ...
    return {
        ...
        "forecast_successful": forecast_success
        if "forecast_success" in locals()
        else False,
